...
public class VoteController extends BaseController{
    ...
    public ModelAndView findpage(VoteDto dto, ExtPageQuery page) throws Exception {
        ModelAndView mv = this.getModelAndView(this.getSessionUserId());
        PageResult<VoteDto> result = voteService.findVoteByParam(dto, page.changeToPageQuery());
    ...
    public ModelAndView detail(String id) throws Exception {
        ModelAndView mv = this.getModelAndView(this.getSessionUserId());
        VoteApp app = voteService.getVoteAppById(id);
        logger.info(JSON.toJSONString(app));
        mv.setViewName("/vote/detail");
        mv.addObject("datas", app);
        return mv;
    }
